...
class ResNet(nn.Module):
    def __init__(self, arch, num_classes=10, layer_type=nn.Conv2d):
        ...
        self.net = nn.Sequential(self.b1(3, 64, layer_type=layer_type))
    ...
    @staticmethod
    def b1(in_channels: int, out_channels: int, layer_type=nn.Conv2d):
    ...
    @staticmethod
    def block(num_residuals, prev_num_channels, num_channels, first_block=False, layer_type=nn.Conv2d):
        blk = []
        for i in range(num_residuals):
            if i == 0 and not first_block:
                blk.append(
                    Residual(prev_num_channels, num_channels, use_1x1conv=True, strides=2, layer_type=layer_type)
                )
            else:
                blk.append(Residual(num_channels, num_channels, layer_type=layer_type))
        return nn.Sequential(*blk)


class ResNet18(ResNet):
    def __init__(self, num_classes=10, layer_type=nn.Conv2d):
        super().__init__(
            [
                (2, 64, 64),
                (2, 64, 128),
